...
namespace ResolveMACList
{
    ...
    public class IPInfo
    {
        ...
        public static IPInfo GetIPInfo(List<IPInfo> arpdata, string macAddress)
        {
            var ipinfo = (from ip in arpdata
                          where ip.MacAddress.ToLowerInvariant() == macAddress.ToLowerInvariant()
                          select ip).FirstOrDefault();
        ...
        public static bool Contains(List<IPInfo> arpdata, string mac)
        {
            return arpdata.Any(T => T.MacAddress == mac);
        }
